...
import { Plus, Search, Filter, LayoutGrid, List as ListIcon, Calendar, User, Warehouse, Truck } from 'lucide-react'
import dummyData from '@/data/dummy.json'

// Define Delivery Type based on dummy data
type Delivery = typeof dummyData.deliveries[0]

export default function DeliveriesListPage() {
  const router = useRouter()
  const [deliveries, setDeliveries] = useState<Delivery[]>(dummyData.deliveries as Delivery[])
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-primary">Deliveries</h1>
          <p className="text-sm text-text-secondary mt-1">Manage outgoing stock and customer shipments.</p>
        </div>
        <Button
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-background-secondary/30 hover:bg-background-secondary/30">
                ...
                <TableHead className="text-xs font-semibold">From (Warehouse)</TableHead>
                <TableHead className="text-xs font-semibold">To (Customer)</TableHead>
              ...
              {filteredDeliveries.length === 0 ? (
                ...
                filteredDeliveries.map(delivery => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge variant={getStatusBadgeVariant(delivery.status)} className="text-xs capitalize">
                        {delivery.status}
                      </Badge>
                    </TableCell>
                  </TableRow>
          ...
          {['draft', 'waiting', 'ready', 'done'].map(status => (
            <div key={status} className="space-y-3">
              ...
              <div className="space-y-2.5">
                {filteredDeliveries.filter(d => d.status === status).length === 0 ? (
                  ...
                  filteredDeliveries.filter(d => d.status === status).map(delivery => (
                    <div
                      key={delivery.id}
                    ...
                    >
                      ...
                      <div className="space-y-1.5 text-xs">
                        <div className="flex items-center gap-1.5 text-text-secondary">
                          ...
                          <span className="truncate">{delivery.warehouse}</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-text-secondary">
                          <Truck className="h-3 w-3 text-text-tertiary" />
