...
import { ArrowLeft, Printer, X, Check, Plus, Trash2, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function DeliveryDetailPage() {
  ...
  useEffect(() => {
    if (id === 'new') {
      ...
      const existingDelivery = dummyData.deliveries.find(d => d.id === id);
      if (existingDelivery) {
        setStatus(existingDelivery.status as DeliveryStatus);
        setFormData({
          customer: existingDelivery.customer,
          warehouse: existingDelivery.warehouseId,
          scheduleDate: existingDelivery.scheduledDate,
          responsible: existingDelivery.deliveredByName || 'Mitchell Admin',
          deliveryAddress: existingDelivery.deliveryAddress,
          operationType: 'delivery'
        });

        // Map items and check stock
        const mappedItems = existingDelivery.items.map((item, index) => {
  ...
  const handleCheckAvailability = () => {
    ...
    if (allAvailable) {
      ...
      setStatus('waiting');
      alert('Some items are not in stock. Status set to Waiting.');
    }
  };
  ...
  const updateItem = (itemId: string, field: keyof DeliveryItem, value: any) => {
    setItems(items.map(i => {
      if (i.id === itemId) {
        ...
        // If product changed, update availability (mock logic)
        if (field === 'productId') {
  ...
  const deliveryNumber = id === 'new' ? 'New Delivery' :
    dummyData.deliveries.find(d => d.id === id)?.deliveryNumber || 'WH/OUT/0001';
    ...
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 text-sm text-text-tertiary">
          <span className="cursor-pointer hover:text-primary transition-colors" onClick={() => router.push('/operations/deliveries')}>Operations</span>
          <span>/</span>
          <span className="cursor-pointer hover:text-primary transition-colors" onClick={() => router.push('/operations/deliveries')}>Deliveries</span>
          <span>/</span>
          <span className="font-medium text-primary">{id === 'new' ? 'New' : deliveryNumber}</span>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-border bg-background-secondary/20">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <h1 className="text-xl md:text-2xl font-bold text-primary">{deliveryNumber}</h1>
        ...
        <div className="px-6 py-3 bg-background-secondary/30 border-b border-border">
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className={`font-medium transition-colors ${status === 'waiting' ? 'text-warning' : ['ready', 'done'].includes(status) ? 'text-success' : 'text-text-tertiary'}`}>
              Waiting
            </span>
            <div className={`h-px flex-1 max-w-[60px] ${['ready', 'done'].includes(status) ? 'bg-success' : 'bg-border'}`} />
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Modern Homes Inc">Modern Homes Inc</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium text-text-secondary">Source Warehouse</Label>
              <Select
                disabled={!isEditable}
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-background-secondary/30 hover:bg-background-secondary/30">
                  ...
                  <TableHead className="h-10 text-xs font-semibold w-[140px]">Available</TableHead>
